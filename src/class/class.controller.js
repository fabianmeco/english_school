...
exports.put = function (req, res) {
    joi.validate(req.body, schema_update)
        .then(function () {
            let fecha = req.body.date || req.clss.date;
            let date = new Date(fecha);
            hapi.holidays({
                country: 'CO',
                year: date.getFullYear(),
                month: date.getMonth() + 1,//left implementation
                day: date.getDay() + 1
            }, function (err, data) {
                if (data.holidays) {
                    console.log('Fails holiday');
                    return res.status(422).send({ "name": "error", "message": "Forbiden This is a holiday" });
                }
                if (date.getDate() === 6) {
                    console.log('Fails sunday');
                    return res.status(422).send({ "name": "error", "message": "Bad hour request" });
                }
                //validate if hour is o'clock
                if (date.getMinutes() !== 0) {
                    console.log('Fails oclock');
                    return res.status(422).send({ "name": "error", "message": "Bad hour request" });
                }
                ...
                Promise.all([
                    ...
                    classModel.findAll({ date: req.body.date || req.clss.date, room_id: req.body.room_id || req.clss.room_id }),
                    //finds if the teacher is busy at this hour
                    classModel.findAll({ date: req.body.date || req.clss.date, teacher_id: req.body.teacher_id || req.clss.teacher_id }),
